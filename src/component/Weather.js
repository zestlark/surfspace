...
const Weather = () => {
    ...
    useEffect(() => {
        const fetchWeather = async () => {
            const data = await fetch('https://api.weatherapi.com/v1/current.json?key=3d1c17ce26b6459cada53111222506&q=malad&aqi=no');
        ...
        return (
            <div className='w-full md:max-w-[340px] p-1 rounded-full flex gap-3 items-center justify-between md:justify-end'>
                ...
                <div>
                    ...
                    <small className='truncate'>{weather?.location.name} , {weather?.location.region} , {weather?.location.country}</small>
                </div>
