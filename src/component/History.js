        ...
        <div className='mt-4 md:mt-8'>
            ...
            <div className='flex justify-start items-start flex-wrap gap-2 mt-2'>
                {apphistory.slice(0, 5).map(history => (
                    <span onClick={() => { addHistoryToSearch(history.search) }} className='border border-gray-500 px-3 inline-flex items-center gap-2 py-1 rounded-full text-sm max-w-full cursor-pointer' key={history.id}><p className='truncate'>{history.search}</p> <i className="ri-arrow-left-up-line text-xl opacity-80"></i></span>
                ))}
